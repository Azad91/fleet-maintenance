...
class ComplaintsImport implements OnEachRow, WithHeadingRow, WithValidation
{
    public function onRow(Row $row)
    {
        ...
        // DQN - ə görə bus_id - ni tap
        $bus = Bus::where('dqn', $rowArray['bus_dqn'])->first();
        if (!$bus) {
            return; // Avtobus yoxdursa, keç
        }

        // Anbar miqdarını yenilə
        $depo_miqdari = null;
        if (!empty($rowArray['detal_kodu']) && !empty($rowArray['islenen_miqdar']) && $rowArray['islenen_miqdar'] > 0) {
            $warehouse = Warehouse::where('kod', $rowArray['detal_kodu'])->first();
            if ($warehouse) {
                $warehouse->miqdar = $warehouse->miqdar - $rowArray['islenen_miqdar'];
                $warehouse->save();
        ...
        Complaint::create([
            ...
            'detal_kodu' => $rowArray['detal_kodu'] ?? null,
            'detal_adi' => $rowArray['detal_adi'] ?? null,
